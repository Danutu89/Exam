<script lang="ts">
	...
	const accountOptions: Array<{ value: string; text: string; callback?: () => void }> = [
		{ value: 'companies', text: 'Companies', callback: () => goto('/companies') },
		{ value: 'account', text: 'Account', callback: () => goto('/account') },
...
<Navbar>
	<Item logo link to="/">New Era</Item>
	...
	<Group dir="right">
		...
		<Item link to="/companies" selected={$page.path === '/companies'}>Companies</Item>
